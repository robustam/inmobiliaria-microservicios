...
public class GlobalExceptionHandler {
    ...
    // Ejemplo: puntuacion fuera de rango 1-5
    // Ejemplo: comentario muy corto
    @ExceptionHandler(MethodArgumentNotValidException.class)
    public ResponseEntity<?> handleValidationErrors(
            MethodArgumentNotValidException ex) {

        log.warn("Error de validacion en resena-service: {}",
                ex.getMessage());
    ...
    // Ejemplo: usuario ya reseno esta propiedad
    // Ejemplo: resena no encontrada
    @ExceptionHandler(RuntimeException.class)
    public ResponseEntity<?> handleRuntimeException(
            RuntimeException ex) {

        log.error("Error de negocio en resena-service: {}",
                ex.getMessage());
    ...
    public ResponseEntity<?> handleGenericException(
        ...
        log.error("Error inesperado en resena-service: {}",
                ex.getMessage());
